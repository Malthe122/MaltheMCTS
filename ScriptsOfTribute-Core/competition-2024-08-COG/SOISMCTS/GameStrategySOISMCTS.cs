...
public class GameStrategySOISMCTS
{
    ...
    public GameStrategySOISMCTS(int cardCount, GamePhase currentGamePhase)
    {
    ...
    double CardsValues(SeededGameState gameState)
    {
        ...
        foreach (SerializedAgent agent in gameState.EnemyPlayer.Agents)
        {
            ...
            value += AgentTierSOISMCTS.GetCardTier(agent.RepresentingCard.CommonId) * GetWeight(Param.EnemyAgent) * (((double)agent.CurrentHp + 2) / ((double)agent.RepresentingCard.HP) + 2);
        ...
        {
            value += HandTierListSOISMCTS.GetCardTier(card.CommonId) * GetWeight(Param.UpcomingCard);
            if (ourCombos.ContainsKey(card.Deck))
            {
